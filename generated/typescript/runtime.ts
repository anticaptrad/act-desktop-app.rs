...
export function loadFrom(lookup: (key: string) => string | undefined): DesktopEnvValues {
  return {
    act_desktop_app_bind: nonEmpty(lookup("ACT_DESKTOP_APP_BIND")) ?? "127.0.0.1:9090",
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const act_desktop_app_bind = pick(["ACT_DESKTOP_APP_BIND"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "127.0.0.1:9090");
  if (act_desktop_app_bind !== undefined) out["ACT_DESKTOP_APP_BIND"] = act_desktop_app_bind;
  return out;
}
